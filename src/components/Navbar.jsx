...
const CustomMobileLink = ({ href, title,toggle, className = "" }) => {
  ...
  return (
    <button
      href={href}
      className={`${className} relative group text-light dark:text-dark my-2 text-2xl font-bold`}
      onClick={handleClick}
    >
      {title}
      <span
...
const Navbar = () => {
  ...
  return (
    <header className="w-full px-32 py-8 font-medium flex items-center justify-between z-10 dark:text-light relative lg:px-16 md:px-12 sm:px-8">
      <button
      ...
      {isOpen ? (
        <motion.div className="min-w-[80vw] min-h-[80vh] flex flex-col justify-between items-center fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-30 bg-dark/90 dark:bg-light/85 rounded-lg backdrop:blur-md py-32"
